...
int main() {
	...
	if (DATASET == PERFORMANCE_DATASET) {
		...
		path_right = "/home/yonghui/Datasets/kitti_odom/sequences/09/image_3/*.png";
		path_disp = "/home/yonghui/Datasets/kitti_odom/sequences/09/image_3/*.png";
	}
	...
	int num_files = filenames_disp.size();
	...
	//#######################
	int range = ALL;               //<<<<-------------- // set here range to 'ALL', 'ONE' or a specific number
	//#######################
	...
	std::cout << "image range: " << range << std::endl;
	for (size_t i=0; i<range; i++) {
		if (single_image) i = 9;
		
		boost::posix_time::ptime time_start = boost::posix_time::microsec_clock::local_time();

		if (DATASET == PERFORMANCE_DATASET) pipeline(filenames_left[i], filenames_right[i], " ", statistics);
		...
		if (DATASET == ACCURACY_DATASET) cv::waitKey(1);
